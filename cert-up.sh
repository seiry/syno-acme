...
installAcme () {
  ...
  ACME_SH_ADDRESS='https://github.com/acmesh-official/acme.sh/archive/refs/tags/3.0.5.tar.gz'
...
generateCrt () {
  ...
  ${ACME_BIN_PATH}/acme.sh --force --log --issue --dns ${DNS} --dnssleep ${DNS_SLEEP} -d "${DOMAIN}" -d "*.${DOMAIN}" -k  ec-384 --server letsencrypt
